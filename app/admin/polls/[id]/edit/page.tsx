...
export default function EditPollPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadPollData = async () => {
      try {
        ...
        const pollData = mockPollData

        setPollName(pollData.name)
        setQuestion(pollData.question)
        setCategory(pollData.category)
        setType(pollData.type)
        setIsActive(pollData.isActive)
        setAllowMultiple(pollData.allowMultiple)
        setEndDate(pollData.endDate)
        setOptions(pollData.options)
        setTotalVotes(pollData.totalVotes)
        setAssociatedContentId(pollData.associatedContentId || null)
        if (pollData.associatedContentId) {
          let name = null
          if (pollData.type === "article") name = mockArticles.find((a) => a.id === pollData.associatedContentId)?.title
          else if (pollData.type === "video")
            name = mockVideos.find((v) => v.id === pollData.associatedContentId)?.title
          else if (pollData.type === "event")
            name = mockEvents.find((e) => e.id === pollData.associatedContentId)?.title
          else if (pollData.type === "page") name = mockPages.find((p) => p.id === pollData.associatedContentId)?.title
          setAssociatedContentName(name || null)
        }
      } catch (error) {
        toast({
          title: "Error",
          description: "Gagal memuat data polling.",
        })
      } finally {
        setIsLoading(false)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Berhasil!",
        description: "Polling berhasil diperbarui.",
      })
      ...
      toast({
        title: "Error",
        description: "Gagal memperbarui polling. Silakan coba lagi.",
      })
    } finally {
      ...
      <div className="px-[10%] py-8">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Edit Polling</h1>
              <p className="text-gray-600 mt-1">
                Perbarui polling - Total votes: <span className="font-medium">{totalVotes}</span>
              </p>
            </div>
          </div>
                        ...
                        <div className="flex-1">
                          <Label htmlFor={`option-${option.id}`}>
                            ...
                            {option.votes !== undefined && (
                              <span className="text-sm text-gray-500 ml-2">({option.votes} votes)</span>
                            )}
                          </Label>
                          <Input
                            id={`option-${option.id}`}
